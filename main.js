...
async function loadCSVFromGitHub() {
  const url = "https://api.github.com/repos/amina-commits/search_engine/contents/applications.csv";
  ...
  console.log("Loaded CSV from GitHub (raw API). Size:", csvText.length);

  await processCSVWithProgress(csvText, (rows, total) => {
    const info = document.getElementById("upload-info");
    if (info) info.textContent = `Loading default CSV: ${rows}/${total}`;
  });

  buildSortedTermsCache();
  console.log("CSV loaded and indexed successfully.");
}
